...
def generate_report_with_gemini(context_data, report_type):
    ...
    if report_type == "周度":
        # --- 周报模式：深度介绍核心项目 ---
        task_prompt = """
        1. 标题要具有吸引力，可以包含本周1-2个热点项目名称。
        2. 开头1-2句话概括和引入。
        3. 本周热门项目速览。按照star排序挑选10个项目，对每个项目用一句话极其精简地概括其核心功能，star数。格式：项目名称（star数⭐|语言）：概括。
        4. 深度点评这 10 个项目，注明项目的发表时间、star数⭐、仓库地址URL，深度点评，并重点分析它们的【实际应用场景】。
        5. 总结本周的趋势的概要，2-3点。
        """
    elif report_type == "月度":
        # --- 月报模式：广度概括 + 精选点评 ---
        task_prompt = """
        1. 标题要具有吸引力，可以包含本月1-2个热点项目名称。
        2. 开头1-2句话概括和引入。
        3. 本月热门项目速览。按照star排序挑选10个项目，对每个项目用一句话极其精简地概括其核心功能，star数。格式：项目名称（star数⭐|语言）：概括。
        4. 深度点评这 10 个项目，注明项目的发表时间、star数⭐、仓库地址URL，深度点评，并重点分析它们的【实际应用场景】。
        5. 总结本月的趋势的概要，2-3点。
        """
